...
public class BRegistries {
    public static final ResourceKey<Registry<SyringeFluidType>> SYRINGE_FLUIDS =
            key("syringe_fluids");
    public static final ResourceKey<Registry<SyringeExtractionType>> SYRINGE_EXTRACTION =
            key("syringe_extraction");
    public static final ResourceKey<Registry<EngineFluidType>> ENGINE_FLUIDS =
            key("engine_fluids");

    private static <T> ResourceKey<Registry<T>> key(String name) {
        return ResourceKey.createRegistryKey(BloodIsFuel.path(name));
    }
}
