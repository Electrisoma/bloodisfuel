...
public class BloodIsFuelJEI implements IModPlugin {
    ...
    @Override public <T> void registerFluidSubtypes(ISubtypeRegistration registration, IPlatformFluidHelper<T> platformFluidHelper) {
        ...
        Set<Fluid> syringeFluids = SyringeFluidTypeManager.getAll(access).stream()
                .flatMap(type -> type.fluids().stream())
                .flatMap(HolderSet::stream)
                .map(Holder::value)
                .filter(fluid -> {
                    ResourceLocation key = ForgeRegistries.FLUIDS.getKey(fluid);
                    return key != null && !key.getPath().contains("flowing");
                })
                .collect(Collectors.toSet());

        for (Fluid fluid : syringeFluids) {
            registration.registerSubtypeInterpreter(
                    ForgeTypes.FLUID_STACK,
                    fluid,
                    (fluidStack, context) -> SyringeJeiHelper.getNormalizedFluidKey(fluidStack)
            );
        }
    }
